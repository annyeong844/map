...
import { computeFanIn } from './fan-in.js';
import { scanIndexDrift } from './index-drift.js';
import { resolvePythonBackend, } from './python-command.js';
import { INDEX_VERSION, } from './types.js';
import { buildLineIndex, firstLine, indexedLineAt, isRecord, token, } from './util.js';
...
function isPyParse(value) {
    ...
    return (Object.values(value.fileImports).every((edges) => Array.isArray(edges) && edges.every(isPyImportEdge)) && Object.values(value.fileRefs).every(isNumberRecord));
}
...
function isIndexNumber(value) {
    return typeof value === 'number' && Number.isSafeInteger(value) && value >= 0;
}
function isNativePyEnvelope(value) {
    return (isRecord(value) &&
        value.v === 1 &&
...
function isNativePyFileRecord(value) {
...
function isNativePyImportEdge(value) {
    return (Array.isArray(value) &&
        value.length === 2 &&
        typeof value[0] === 'string' &&
        typeof value[1] === 'string');
}
function isNativePyEntry(value) {
    return (Array.isArray(value) &&
...
function isNativePyInvalidRecord(value) {
    return (Array.isArray(value) &&
        value.length === 2 &&
        typeof value[0] === 'string' &&
        typeof value[1] === 'string');
...
function decodeNativePyParse(value) {
    if (!isNativePyEnvelope(value))
        return null;
    const result = {
        entries: [],
        fileImports: {},
    ...
    for (const record of value.p) {
        ...
        for (const edge of wireImports) {
            if (!isNativePyImportEdge(edge))
                return null;
            imports.push({ source: edge[0], name: edge[1] });
        }
        for (const entry of wireEntries) {
            if (!isNativePyEntry(entry))
                return null;
            const [name, kindIndex, charStart, charEnd, line, endLine, searchText, anchorOffset, namePath, className, extendsName,] = entry;
            result.entries.push({
                name,
...
function partitionPyTargets(targets, stats) {
    let totalBytes = 0;
    ...
    const availableMemory = typeof process.availableMemory === 'function'
        ...
        : freemem();
    const memoryShards = Math.max(1, Math.floor(Math.max(0, availableMemory - PY_MEMORY_RESERVE) / PY_MEMORY_PER_SHARD));
    const shardCount = Math.min(MAX_PY_SHARDS, availableParallelism(), memoryShards, targets.length, Math.max(1, Math.ceil(totalBytes / PY_BYTES_PER_SHARD)));
    if (shardCount <= 1)
        return [targets];
    ...
    for (const target of weighted) {
        ...
        for (let i = 1; i < shards.length; i++) {
            if (shards[i].bytes < lightest.bytes)
                lightest = shards[i];
        }
        lightest.bytes += target.bytes;
        lightest.targets.push(target);
    }
    return shards.map((shard) => shard.targets.sort((a, b) => a.order - b.order).map(({ file }) => file));
}
function mergePyParses(parts) {
    const merged = {
        entries: [],
        fileImports: {},
        fileTokens: {},
    ...
    for (const part of parts) {
        ...
        for (const file of part.filesMissing)
            merged.filesMissing.push(file);
        for (const file of part.filesInvalid)
            merged.filesInvalid.push(file);
    }
    return merged;
}
function runPyShard(backend, root, files, targets, signal) {
    return new Promise((res, rej) => {
        if (signal?.aborted) {
            rej(new Error('Index build aborted.'));
            return;
        }
        const backendArgs = backend.kind === 'native'
            ? [...backend.args, root]
        ...
        const fail = (reason) => {
            ...
            rej(new Error(`Python backend failed (${backend.display}): ${reason}. ` +
                (backend.kind === 'native'
        ...
        p.on('close', (code) => {
            ...
            try {
                const parsed = JSON.parse(Buffer.concat(stdout, stdoutBytes).toString());
                let normalized;
...
async function runPyBackend(root, files, targets, stats, signal) {
    let backend;
    try {
    ...
    if (signal?.aborted)
        controller.abort();
    try {
        const parts = await Promise.all(shards.map((targetsInShard) => runPyShard(backend, root, files, targetsInShard, controller.signal)));
...
async function readAll(root, files, concurrency = READ_CONCURRENCY) {
    const out = new Map();
    let i = 0;
    const worker = async () => {
...
function sameImportEdges(before, after) {
...
const fanInSurfaceKey = (entry) => entry.kind === 'ClassMethod' || entry.namePath
    ? null
    : `${entry.name}\0${entry.default ? 1 : 0}`;
function sameFanInSurface(before, after) {
...
function canReuseFanIn(previous, files, changedFiles, prevByFile, entriesByFile, fileImports) {
    if (!previous)
        return false;
    ...
    for (const file of changedFiles) {
        ...
        if (before.some((entry) => typeof entry.fanIn !== 'number'))
            return false;
        if (!sameImportEdges(previous.fileImports?.[file] ?? [], fileImports[file] ?? [])) {
            return false;
        }
        if (!sameFanInSurface(before, after))
            return false;
    }
    return true;
}
function compareWithinFile(a, b) {
...
export async function buildIndex(opts) {
        ...
        if (!counts) {
            ...
            for (const entry of prev.entries) {
                if (entry.kind === 'ClassMethod')
                    methods++;
                else if (entry.namePath)
                    nestedDefs++;
                else if (entry.visibility === 'module-private')
                    privateDefs++;
            }
            counts = {
    ...
    const filesInvalid = new Set((prev?.meta.invalidFiles ?? []).filter((file) => currentFiles.has(file)));
    const usedIds = new Map();
    const mkId = (file, name, kind, line) => {
        let id = `${file}#${name}`;
        if (usedIds.has(id))
            id = `${file}#${name}#${kind}`;
        if (usedIds.has(id))
    ...
    if (prev) {
        for (const entry of prev.entries) {
            ...
            if (bucket)
                ...
                prevByFile.set(entry.file, [entry]);
        }
    ...
    if (fanInReused) {
        for (const file of changedFiles) {
            const oldFanIn = new Map();
            for (const entry of prevByFile.get(file) ?? []) {
                const key = fanInSurfaceKey(entry);
                if (key !== null)
                    oldFanIn.set(key, entry.fanIn ?? 0);
            }
        ...
        const importsByFile = new Map(Object.entries(fileImports));
    ...
    let nestedDefs = 0;
    for (const e of entries) {
        if (e.kind === 'ClassMethod')
            methods++;
        else if (e.namePath)
            nestedDefs++;
        else if (e.visibility === 'module-private')
            privateDefs++;
    }
