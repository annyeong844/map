...
function definitionFile(definition) {
    return typeof definition === 'string' ? definition : definition.file;
}
function definitionName(definition, requestedName) {
...
function simpleRelativeBase(fromFile, source) {
    if (source.length <= 2 ||
    ...
    for (let i = 2; i < source.length - 1; i++) {
        if (source.charCodeAt(i) !== SLASH_CHAR_CODE)
            continue;
        const next = source.charCodeAt(i + 1);
        if (next === DOT_CHAR_CODE || next === SLASH_CHAR_CODE)
            return null;
    }
 ...
 */
function resolveRelative(fromFile, source, fileSet) {
    ...
    if (!source.startsWith('./') && !source.startsWith('../'))
        return null;
    const base = simpleRelativeBase(fromFile, source) ??
 ...
 */
export function computeFanIn(files, importsByFile) {
    ...
    const targetFor = (fromFile, source) => {
    ...
    const dominantExactNames = new Set();
    ...
    for (const [f, edges] of importsByFile) {
        ...
        for (let order = 0; order < edges.length; order++) {
            ...
            if (edge.name === '*') {
                ...
                if (!routes.exact.has(edge.name))
                    routes.exact.set(edge.name, route);
            }
    ...
    const collapseWildcardPath = (start, stops, ignoreDominantExact) => {
        ...
        for (let i = path.length - 1; i >= 0; i--) {
            if (!stops.has(path[i]))
                stops.set(path[i], result);
        }
        return result;
    };
    const unconditionalStops = new Map();
    /** Collapse pure `export *` chains once for every name. */
    const skipUnconditional = (start) => collapseWildcardPath(start, unconditionalStops, false);
    ...
    const wildcardStops = new Map();
    const skipWildcard = (start) => collapseWildcardPath(start, wildcardStops, true);
    const rebaseDefinition = (definition, requestedName, exposedName) => {
        const terminalName = definitionName(definition, requestedName);
        if (terminalName === exposedName)
            return definitionFile(definition);
    ...
    const resolveDef = (start, requestedName) => {
        ...
        for (;;) {
            ...
            if (cycleAt !== undefined) {
                const cycle = path.slice(cycleAt);
                const terminal = cycle.reduce((best, candidate) => `${candidate.file}\0${candidate.name}` < `${best.file}\0${best.name}`
                    ? candidate
                    : best);
    ...
    const resolveDefs = (start, names) => {
        const results = new Map();
        const active = new Set();
        let wildcardTerminal;
            ...
            if (routes?.exact) {
                for (const [name, exact] of routes.exact) {
                    ...
                    const terminal = exact.target
                        ? rebaseDefinition(resolveDef(exact.target, sourceName), sourceName, name)
                        : file;
                    resolvedDefs.set(`${start}\0${name}`, terminal);
                    results.set(name, terminal);
                }
    ...
    const importers = new Map(); // "defFile::name" -> unique importing files
    for (const [target, byName] of requests) {
        ...
        for (const [name, consumers] of byName) {
            ...
            if (existing === undefined) {
                ...
                for (const fromFile of consumers)
                    existing.add(fromFile);
            }
        }
    }
    const fanIn = new Map();
