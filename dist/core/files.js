...
import { isParseable } from './extract-symbols.js';
import { posix } from './util.js';
...
export function gitFileListCommand(root, platform = process.platform) {
    const unc = platform === 'win32'
        ? /^(?:\\\\|\/\/)(?:wsl\.localhost|wsl\$)[\\/]([^\\/]+)(?:[\\/](.*))?$/iu.exec(root)
        : null;
    if (unc) {
 ...
 */
export async function listSourceFiles(root, signal) {
...
async function gitFiles(root, signal) {
    try {
...
async function walkFiles(root, signal) {
    ...
    for (let cursor = 0; cursor < dirs.length;) {
        ...
        cursor += batch.length;
        const listings = await Promise.all(batch.map(async ({ dir, rel }) => {
            try {
                return {
                    dir,
                    rel,
