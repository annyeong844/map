...
async function statAll(root, files, signal, concurrency = 64) {
    const out = new Map();
    let cursor = 0;
    const worker = async () => {
...
function sameFile(previous, current) {
    return (!!previous &&
        !!current &&
...
export async function scanIndexDrift(rootInput, previousInput, force = false, signal) {
    ...
    for (const file of files) {
        if (++compared % SCAN_YIELD_INTERVAL === 0)
    ...
    for (const file in priorStats) {
        if (++compared % SCAN_YIELD_INTERVAL === 0)
            await scheduler.yield();
        signal?.throwIfAborted();
        if (Object.hasOwn(priorStats, file) && !stats.get(file))
            removed++;
    }
    return {
        root,
