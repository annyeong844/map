import { exactFileEntryRange, getPreparedLookup } from './store.js';
const searchTables = new WeakMap();
const resultCaches = new WeakMap();
...
function kindPreference(kind, verbQuery) {
    if (!verbQuery)
        return 0;
...
function queryTerms(q) {
    ...
    for (const raw of q.split(/[^A-Za-z0-9_$]+/)) {
        ...
        if (low.length < 2 || STOP.has(low))
            continue;
        out.push({ raw, low });
    }
    return out;
}
const isUpperCode = (code) => code >= ASCII_UPPER_A && code <= ASCII_UPPER_Z;
const isLowerCode = (code) => code >= ASCII_LOWER_A && code <= ASCII_LOWER_Z;
const isDigitCode = (code) => code >= ASCII_DIGIT_0 && code <= ASCII_DIGIT_9;
const isWordCode = (code) => isUpperCode(code) || isLowerCode(code) || isDigitCode(code);
...
function hasSubwordStreaming(name, low) {
    let start = -1;
    for (let i = 0; i <= name.length; i++) {
        const code = i < name.length ? name.charCodeAt(i) : -1;
        if (!isWordCode(code)) {
            if (start !== -1 && sameLowerAscii(name, start, i, low))
                return true;
            start = -1;
            continue;
        }
        ...
        const boundary = isUpperCode(code) &&
            (isLowerCode(prev) ||
                isDigitCode(prev) ||
                (isUpperCode(prev) && isLowerCode(next)));
        if (boundary) {
            if (sameLowerAscii(name, start, i, low))
                return true;
            start = i;
        }
    }
...
function sameLowerAscii(name, start, end, low) {
    if (end - start !== low.length)
        return false;
    for (let i = 0; i < low.length; i++) {
        let code = name.charCodeAt(start + i);
        if (isUpperCode(code))
            code += ASCII_CASE_OFFSET;
        if (code !== low.charCodeAt(i))
            return false;
    }
    return true;
}
...
function subwordBoundaryMask(name) {
    if (name.length > MAX_WORD_MASK_NAME_LENGTH)
        return null;
    let mask = 0;
    let start = -1;
    for (let i = 0; i <= name.length; i++) {
...
function isMaskedSubword(mask, start, end) {
    if ((mask & (1 << start)) === 0 || (mask & (1 << end)) === 0)
        return false;
    if (end <= start + 1)
        return true;
...
function hasSubword(name, nameLow, low, firstAt, normalized) {
    if (name.length !== nameLow.length)
        return hasSubwordStreaming(name, low);
    const mask = normalized.wordMask === undefined
        ? (normalized.wordMask = subwordBoundaryMask(name))
        : normalized.wordMask;
    if (mask === null)
        return hasSubwordStreaming(name, low);
    for (let at = firstAt; at !== -1; at = nameLow.indexOf(low, at + 1)) {
        if (isMaskedSubword(mask, at, at + low.length))
            return true;
    }
    return false;
}
...
function compareCandidateToScored(entry, tier, matched, kindPrior, fanIn, lenPen, other) {
...
function toHits(scored) {
    return scored.map((s) => ({
        id: s.entry.id,
        name: s.entry.name,
        namePath: s.entry.namePath ??
...
function resultCacheKey(query, opts, limit) {
    const kind = opts.kind ?? '';
    const file = opts.file ?? '';
    return `${limit}:${kind.length}:${kind}:${file.length}:${file}:${query}`;
}
function cachedResult(index, key) {
...
function rememberResult(index, key, scored) {
...
function offerTop(heap, value, limit) {
    if (heap.length < limit) {
        heap.push(value);
        let i = heap.length - 1;
        while (i > 0) {
            const parent = (i - 1) >>> 1;
            if (compareScored(heap[i], heap[parent]) <= 0)
                break;
            const swap = heap[i];
            heap[i] = heap[parent];
            heap[parent] = swap;
    ...
    if (compareScored(value, heap[0]) >= 0)
        return;
    heap[0] = value;
    let i = 0;
    for (;;) {
        ...
        if (compareScored(heap[worse], heap[i]) <= 0)
            return;
        const swap = heap[i];
        heap[i] = heap[worse];
        heap[worse] = swap;
...
function offerEntry(heap, entry, tier, match, matched, kindPrior, lenPen, fanIn, limit) {
    if (heap.length >= limit &&
        compareCandidateToScored(entry, tier, matched, kindPrior, fanIn, lenPen, heap[0]) >= 0) {
        return;
    }
    offerTop(heap, { entry, tier, match, matched, kindPrior, lenPen, fanIn }, limit);
}
/**
 ...
 */
function locateScored(index, query, opts = {}) {
    ...
    const scored = []; // bounded to `limit` by offerTop
    ...
    if (exactBucket) {
        ...
        if (scored.length) {
            ...
            if (cacheKey)
                rememberResult(index, cacheKey, scored);
            return scored;
        }
    }
    ...
    for (let i = start; i < end; i++) {
        ...
        offerEntry(scored, e, best, tierLabel(best), matched, kindPrior, lengthPenalty(e.name, namePart), e.fanIn ?? 0, limit);
    }
    scored.sort(compareScored);
    if (cacheKey)
        rememberResult(index, cacheKey, scored);
...
export function locate(index, query, opts = {}) {
...
export function locateWithEntries(index, query, opts = {}) {
    const scored = locateScored(index, query, opts);
    return { hits: toHits(scored), entries: scored.map((item) => item.entry) };
}
function termTier(name, nl, normalized, t) {
    if (name === t.raw)
        return TIER_EXACT;
    if (nl === t.low)
        return TIER_CASE_INSENSITIVE_EXACT;
    const at = nl.indexOf(t.low);
    if (at !== -1) {
        if (hasSubword(name, nl, t.low, at, normalized))
            return TIER_WORD; // whole subword: "diff" in computeDiff
        if (at === 0)
            return TIER_PREFIX;
...
function lengthPenalty(name, q) {
    return Math.min(MAX_LENGTH_PENALTY, Math.max(0, name.length - q.length) * LENGTH_PENALTY_FACTOR);
}
