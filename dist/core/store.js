...
import { ORDERED_ENTRIES_VERSION, } from './types.js';
import { isRecord } from './util.js';
export const DEFAULT_INDEX_PATH = '.map-index.json';
function hasOnlyOptionalType(value, keys, type) {
...
function isMapEntry(value) {
    return (isRecord(value) &&
        ...
        hasOnlyOptionalType(value, ['namePath', 'className', 'extends', 'visibility', 'definitionId'], 'string') &&
        hasOnlyOptionalType(value, ['endLine', 'charStart', 'charEnd', 'anchorOffset', 'fanIn', 'intraRefs'], 'number') &&
...
function isMapIndex(value) {
    if (!isRecord(value) ||
        ...
        !Object.values(value.fileTokens).every((token) => typeof token === 'string') ||
        !isRecord(value.fileStats) ||
        !Object.values(value.fileStats).every(isFileStat) ||
        !isRecord(value.fileImports) ||
        !Object.values(value.fileImports).every((edges) => Array.isArray(edges) && edges.every(isImportEdge))) {
        return false;
    }
...
export function qualifiedSymbolRef(entry) {
    const namePath = entry.namePath ??
        (entry.className ? `${entry.className}/${entry.name}` : undefined);
    return namePath ? `${entry.file}#${namePath}` : undefined;
}
const lookupTables = new WeakMap();
const siblingTables = new WeakMap();
...
export function prepareLookup(index) {
    ...
    for (let i = 0; i < entries.length; i++) {
        ...
        if (!named)
            ...
            byName.set(entry.name, [named, entry]);
    }
...
export function getPreparedLookup(index) {
...
export function exactFileEntryRange(index, file) {
    if (!file || index.meta.version < ORDERED_ENTRIES_VERSION)
        return undefined;
    const entries = index.entries;
    let lo = 0;
    ...
    while (lo < hi) {
        const mid = (lo + hi) >>> 1;
        if (entries[mid].file < file)
            lo = mid + 1;
        else
            hi = mid;
    }
    if (entries[lo]?.file !== file)
        return undefined;
    const start = lo;
    hi = entries.length;
    while (lo < hi) {
...
export function nextSiblingLine(index, entry) {
    ...
    for (let i = 0; i < siblings.length;) {
        ...
        if (next < siblings.length) {
            ...
            for (let j = i; j < next; j++)
                nextLineById.set(siblings[j].id, nextLine);
        }
        i = next;
    }
...
export function saveIndex(index, path = DEFAULT_INDEX_PATH) {
...
export function loadIndex(path = DEFAULT_INDEX_PATH) {
    const indexPath = resolve(path);
    ...
    if (typeof relativeRoot === 'string') {
        ...
        if (existsSync(candidate))
            idx.meta.root = candidate;
    }
    else if (!existsSync(idx.meta.root) &&
