...
export function indexedLineAt(index, off) {
...
export function indexedOffsetOfLine(index, line) {
    if (line <= 1)
        return 0;
...
export function indexOfAll(hay, needle, from = 0, to = hay.length) {
    if (!needle)
        return [];
    const out = [];
    ...
    for (;;) {
        ...
        if (i === -1 || i + needle.length > end)
            break;
        out.push(i);
        cursor = i + needle.length;
        if (out.length > MAX_ANCHOR_MATCHES)
            break; // anchor is too generic to be useful past this
    }
    return out;
}
/** 1-based line number containing char offset `off`. Pass a prebuilt LineIndex
 * when the same text will be queried repeatedly. */
export function lineAt(text, off, index) {
    if (index)
        return indexedLineAt(index, off);
    let line = 1;
...
export function offsetOfLine(text, line, index) {
    if (index)
        return indexedOffsetOfLine(index, line);
    if (line <= 1)
        return 0;
    let seen = 1;
    for (let i = 0; i < text.length; i++) {
...
export function firstLine(text, start, cap = 200) {
    ...
    if (text.charCodeAt(end - 1) === CARRIAGE_RETURN_CODE)
        end--;
