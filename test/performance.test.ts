...
import { exactFileEntryRange, getPreparedLookup, nextSiblingLine, prepareLookup } from '../src/core/store.ts';
import { INDEX_VERSION, type MapEntry, type MapIndex } from '../src/core/types.ts';
...
function syntheticIndex(root: string, entries: MapEntry[], fileTokens: Record<string, string> = {}): MapIndex {
  return {
    meta: { tool: 'code-map', version: INDEX_VERSION, generated: '', builtAtMs: 0, root, entryCount: entries.length, fileCount: Object.keys(fileTokens).length },
    fileTokens,
    fileStats: {},
    fileImports: {},
    entries,
...
test('bounded locate returns the exact same ordering as a full candidate sort', () => {
  const entries: MapEntry[] = Array.from({ length: 5_000 }, (_, i) => ({
    ...
    name: `computeThing${i}`,
    kind: i % 5 === 0 ? 'TSInterfaceDeclaration' : i % 3 === 0 ? 'ClassMethod' : 'FunctionDeclaration',
    file: `src/f${i % 37}.ts`,
    line: i + 1,
  ...
  const bounded = locate(index, 'compute thing', { limit: 20 }).map((hit) => hit.id);
  const exhaustive = locate(index, 'compute thing', { limit: entries.length }).slice(0, 20).map((hit) => hit.id);
...
test('allocation-free subword matching preserves camel, separator, acronym, and fuzzy tiers', () => {
  const names = ['computeDiff', 'compute_diff', 'compute-diff', 'XMLHttpRequest', 'xComputeDiffY', 'İcomputeDiff'];
  ...
  const index = syntheticIndex(process.cwd(), entries);
  const score = (id: string, query: string): number | undefined => locate(index, query, { limit: entries.length }).find((hit) => hit.id === id)?.score;
  ...
  assert.equal(score('f4.ts#xComputeDiffY', 'computediff'), 50, 'a term spanning two camel segments is only a substring');
  assert.equal(score('f0.ts#computeDiff', 'pute'), 50);
  assert.equal(score('f0.ts#computeDiff', 'cpdf'), 30);
  assert.equal(score('f5.ts#İcomputeDiff', 'diff'), 80, 'Unicode lowercase length changes retain the old segment behavior');
});
...
test('fuzzy result reuse is O(1) and bounded instead of retaining arbitrary queries', () => {
  ...
  reads = 0;
  locate(index, 'symbol0');
  assert.equal(reads, 0, 'an identical bounded query reuses its scored entries');

  for (let i = 1; i <= 32; i++) locate(index, `missing-query-${i}`);
  reads = 0;
  locate(index, 'symbol0');
  assert.equal(reads, entries.length, 'the 33rd distinct query evicts the oldest cached result');
});

test('a prepared exact-name lookup skips fuzzy locate candidates', () => {
  const entries: MapEntry[] = [
    { id: 'a.ts#alpha', name: 'alpha', kind: 'FunctionDeclaration', file: 'a.ts', line: 1, searchText: 'function alpha() {}', fanIn: 1 },
    { id: 'c.ts#alpha', name: 'alpha', kind: 'FunctionDeclaration', file: 'c.ts', line: 1, searchText: 'function alpha() {}', fanIn: 2 },
    { id: 'b.ts#alphaHelper', name: 'alphaHelper', kind: 'FunctionDeclaration', file: 'b.ts', line: 1, searchText: 'function alphaHelper() {}', fanIn: 99 },
  ];
  const index = syntheticIndex(process.cwd(), entries);
  prepareLookup(index);
  assert.deepEqual(locate(index, 'alpha').map((hit) => hit.id), ['c.ts#alpha', 'a.ts#alpha']);
  assert.equal(readMany(index, ['alpha'])[0].status, 'ambiguous', 'duplicate exact names remain unresolved');
});
...
test('one-shot exact batch resolves in one index pass without retaining lookup tables', () => {
  ...
  assert.ok(visits <= entries.length + 2 * Math.ceil(Math.log2(entries.length)) + 2, `batch range lookup read ${visits} entries`);
  assert.equal(results.every((result) => result.status === 'exact'), true);
  assert.equal(getPreparedLookup(index), undefined, 'one-shot batches do not retain a full lookup table');
});
...
test('exact path-scoped read and fuzzy locate inspect only that v13 file range', () => {
  ...
  const entries = new Proxy(base, {
    get(target, property, receiver) {
      if (typeof property === 'string' && numeric.test(property)) indexedReads++;
      return Reflect.get(target, property, receiver);
    },
  });
  const index = syntheticIndex(root, entries, { [targetFile]: token(targetText) });
  assert.deepEqual(exactFileEntryRange(index, targetFile), { start: targetBase, end: targetBase + perFile });
  indexedReads = 0;
  const result = read(index, `${targetFile}#symbol${targetBase + perFile - 1}`);
  assert.equal(result.status, 'exact');
  assert.equal(result.raw, `symbol${targetBase + perFile - 1}\n`);
  assert.ok(indexedReads <= 3 * Math.ceil(Math.log2(base.length)) + perFile, `path read touched ${indexedReads} entries`);
  ...
  assert.equal(hits.every((hit) => hit.file === targetFile), true);
  assert.ok(indexedReads <= 3 * Math.ceil(Math.log2(base.length)) + perFile, `path locate touched ${indexedReads} entries`);
});
...
test('line-only batch resolves sibling boundaries and preserves exact slices', () => {
  ...
  assert.equal(aimed.aim?.status, 'hit', 'line-only snippet aim exercises its sibling-bounded range');
...
test('ordered line-only boundaries are logarithmic; legacy unsorted indexes stay correct', () => {
  ...
  assert.ok(indexedReads <= 2 * Math.ceil(Math.log2(base.length)) + 2, `binary lookup read ${indexedReads} entries`);
  const firstReads = indexedReads;
  assert.equal(nextSiblingLine(index, base[4_096]), 2);
  assert.equal(indexedReads, firstReads, 'a repeated line-only read reuses only that entry\'s memoized boundary');

  const legacyEntries: MapEntry[] = [
    { id: 'a.ts#one', name: 'one', kind: 'line', file: 'a.ts', line: 1, searchText: 'one' },
    { id: 'b.ts#one', name: 'one', kind: 'line', file: 'b.ts', line: 1, searchText: 'one' },
    { id: 'a.ts#three', name: 'three', kind: 'line', file: 'a.ts', line: 3, searchText: 'three' },
    { id: 'a.ts#two', name: 'two', kind: 'line', file: 'a.ts', line: 2, searchText: 'two' },
  ];
...
test('cold fuzzy resolution reuses the located entry instead of scanning a third time', () => {
  ...
  const entries = new Proxy(base, {
    get(target, property, receiver) {
      if (property === Symbol.iterator) {
        return function* (): IterableIterator<MapEntry> {
          for (const entry of target) { visits++; yield entry; }
        };
      }
      if (typeof property === 'string' && numeric.test(property)) visits++;
      return Reflect.get(target, property, receiver);
  ...
  assert.equal(visits, count * 2, 'one exact pass + one fuzzy pass; no id-to-entry third scan');
});
...
test('fan-in resolves a 12k wildcard barrel chain iteratively for many names', () => {
  ...
  for (let i = 0; i < depth - 1; i++) imports.set(`f${i}.ts`, [{ source: `./f${i + 1}`, name: '*', reexport: true }]);
  imports.set('use.ts', Array.from({ length: 200 }, (_, i) => ({ source: './f0', name: `symbol${i}` })));
...
test('incremental no-op returns the prior snapshot and a deleted empty file invalidates it', async () => {
  ...
  assert.equal(noOp.index, first.index, 'no-op does not clone/re-sort/recompute an identical index');
  ...
  assert.equal(afterDelete.fanInReused, false, 'a changed file set invalidates import-route reuse');
...
test('incremental fan-in skips body-only graphs but rejects import and definition-surface counterexamples', async () => {
  const root = repo({
    ...
    'use.ts': "import { target, future } from './target';\nexport const used = target;\n",
    'worker.ts': 'export function worker() { return 1 }\n',
  });
  const first = await buildIndex({ root });
  assert.equal(first.fanInReused, false);
  assert.equal(first.index.entries.find((entry) => entry.id === 'target.ts#target')?.fanIn, 1);

  writeFileSync(join(root, 'worker.ts'), 'export function worker() { return 123456 }\n');
  const bodyOnly = await buildIndex({ root, previous: first.index });
  assert.equal(bodyOnly.fanInReused, true);
  assert.equal(bodyOnly.index.entries.find((entry) => entry.id === 'target.ts#target')?.fanIn, 1);

  writeFileSync(join(root, 'target.ts'), 'export function target() { return 1 }\nexport function future() { return 2 }\n');
  const addedDefinition = await buildIndex({ root, previous: bodyOnly.index });
  assert.equal(addedDefinition.fanInReused, false, 'an already-imported new definition needs the discarded fan-in key');
  assert.equal(addedDefinition.index.entries.find((entry) => entry.id === 'target.ts#future')?.fanIn, 1);

  writeFileSync(join(root, 'worker.ts'), "import { target } from './target';\nexport function worker() { return target() }\n");
  const changedImport = await buildIndex({ root, previous: addedDefinition.index });
  assert.equal(changedImport.fanInReused, false);
  assert.equal(changedImport.index.entries.find((entry) => entry.id === 'target.ts#target')?.fanIn, 2);
});
